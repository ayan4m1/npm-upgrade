...
const CHANGELOG_URLS_DB_FILE = path.resolve(
  __dirname,
...
if (!moduleName || !changelogUrl) {
  console.error(
    `Usage: ./${path.basename(__filename)} <moduleName> <changelogUrl>`
  );
  process.exit();
}
...
fs.writeFileSync(
  CHANGELOG_URLS_DB_FILE,
  JSON.stringify(changelogUrls, null, 4) + '\n'
);

console.log(
  `Changelog URL for "${moduleName}" module set to "${changelogUrl}"`
);
